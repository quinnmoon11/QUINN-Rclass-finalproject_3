...
![Fig 2. Map of the range of six-lined racerunner lizards and the museum samples collected with the corresponding catalog number. For this figure, the base map and points was made in R, but the range map, in green, needed to be hand drawn using Inkscape](Images/allsampling.pdf)
...
![Fig 4. Map showing the final samples included in ddRADseq library ](../../Results/final_samples_used.pdf)
...
![I used these maps from R](../../Results/discuss_fig_maps.pdf)
![And simple phylos from R to compile and add detail in inkscape](../../Results/discuss_fig_phylo.pdf)

![Fig. 7 Diagram depicting the predicted settlement pattern and corresponding phylogeny for the two hypothesized origins of the Michigan population- recent anthropogenic introduction (A) or native post-glacial northern dispersal (B).](Images/final_discuss_figure.pdf)
...
```{ojs}
data = FileAttachment("../../Data/Raw_data/Table1_apen.csv").csv({ typed: true })
```
